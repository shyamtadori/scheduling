class Schedule < ActiveRecord::Base
  ...
  belongs_to :job, class_name: 'Job', foreign_key: 'job_idx'
	...
	scope :monthly_schedule, lambda{|start_date, end_date, location_ids| where('location_idx in (?) and schedule_date >= ? and schedule_date <= ?', location_ids, start_date, end_date)}
